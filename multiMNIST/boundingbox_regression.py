...
class ResNet(nn.Module):
    def __init__(self, num_classes=100):
        ...
        self.conv2 = nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.fc2 = nn.Linear(16 * 64 * 64, 128)
...
def train(model, train_loader, optimizer):
    ...
    for batch_idx, (image, label, bb) in enumerate(train_loader): # 기존에 정의한 GPU에 데이터를 할당
        ...
        # Forward
        output, output2 = model(image)       # Input : 이미지 데이터 / Output 계산
        # Backward
        loss = criterion(output, label_temp)     # loss 값 계산
        loss += criterion2(output2, bb.view(-1, 8).float())
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():       # 파라미터 업데이트 방지
        for image, label, bb in test_loader:      # image, label, bounding_box
            ...
            test_loss += criterion2(output2, bb.view(-1, 8).float()).item()
